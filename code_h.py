...
# another copy for original dataset
univ_reg = univ.copy()

univ_reg.columns = univ_reg.iloc[0]
univ_reg = univ_reg[2:]

# loop through all universities in the dataset
for i in range(univ_c.shape[0]):
    
    # check if the university has missing values for numerical attributes
    if univ_c.iloc[i][3:].isnull().sum() > 0:
        
        # get the university data for the numerical attributes
        univ_data = univ_c.iloc[i][3:].tolist()

        # find the closest cluster to the university using the same method as before
        cluster_distances = []
        for c in range(k_opt):
            cluster_data = univ_5[univ_5['Cluster'] == c]
            cluster_mean = cluster_data.mean()
            d = euclidean_distance(cluster_mean, univ_data)
            cluster_distances.append(d)

        closest_cluster = cluster_distances.index(min(cluster_distances))

        # get the mean values for the closest cluster
        closest_cluster_data = univ_5[univ_5['Cluster'] == closest_cluster]
        distances_to_pt = []
        
        for jj in range(len(closest_cluster_data)):
            dist_temp = euclidean_distance(closest_cluster_data.iloc[jj], univ_data)
            distances_to_pt.append(dist_temp)
        ...
        for j in range(len(univ_data)):
            if pd.isna(univ_data[j]):
                univ_c.iloc[i][j+3] = closest_data_points_mean[j]
...
for i in range(univ_c.shape[0]):
    ...
    for att in univ_3.columns:
        if att not in univ_2_categ:
            univ_c.iloc[i][att] = univ_c.iloc[i][att]*(univ_reg[att].max()-univ_reg[att].min())+univ_reg[att].min()

######## now we will RE-DO a lot of stuff from before
...
univ_6 = univ_5.copy()
univ_6["College Name"] = univ_c["College Name"]
univ_6["State"] = univ_c["State"]
univ_6["Public (1)/ Private (2)"] = univ_c["Public (1)/ Private (2)"]
...
# obtain statistics for initial analysis

state_distr = univ_reg['State'].value_counts()
priv_distr = univ_reg['Public (1)/ Private (2)'].value_counts()

state_distr_2 = univ_3['State'].value_counts()
priv_distr_2 = univ_3['Public (1)/ Private (2)'].value_counts()
